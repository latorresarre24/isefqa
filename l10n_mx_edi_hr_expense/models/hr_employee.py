...
class HrEmployee(models.Model):
    ...
    l10n_mx_edi_debit_account_id = fields.Many2one(
        ...
        groups="hr.group_hr_user",
        help="Account defined in the journal to use when the employee receive money for expenses.",
    )
    l10n_mx_edi_credit_account_id = fields.Many2one(
        ...
        domain=[("deprecated", "=", False)],
        groups="hr.group_hr_user",
        help="Account defined in the journal to use when the employee paid an invoice from an expense.",
    )
    l10n_mx_edi_payment_mode = fields.Selection(
        lambda self: self.env["hr.expense"].fields_get().get("payment_mode").get("selection"),
        string="Payment Mode",
        default="own_account",
        groups="hr.group_hr_user",
    ...
    def create_petty_cash_journal(self):
        ...
        for emp in self.filtered(lambda e: not e.journal_id):
            journals = self.env["account.journal"].with_context(journal_for_employee=True)
    ...
    def _compute_expenses(self):
        for employee in self:
            employee.update(
                {
                    "expenses_count": self.env["hr.expense"].search_count([("employee_id", "=", employee.id)]),
                    "sheets_count": self.env["hr.expense.sheet"].search_count([("employee_id", "=", employee.id)]),
                }
            )

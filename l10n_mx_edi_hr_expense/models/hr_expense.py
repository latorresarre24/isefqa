...
class HrExpense(models.Model):
    ...
    l10n_mx_edi_expense_id = fields.Many2one("hr.expense", "Active expense related to this achived record.")
    l10n_mx_edi_expenses_count = fields.Integer("Number of Archived Expenses", compute="_compute_expenses_count")
    ...
    def _compute_expenses_count(self):
        for expense in self:
            expense.l10n_mx_edi_expenses_count = self.search_count(
                [
                    ("l10n_mx_edi_expense_id", "=", expense.id),
    ...
    def functional3(self):
        ...
        return not bool(self._get_duplicated_expenses())

    def _get_duplicated_expenses(self):
    ...
    def functional_errors(self):
        return {
            ...
            4: {
                ...
                "message_ok": _("We have old invoices from this vendor, stayin touch if we need something extra."),
            ...
            7: {
                ...
                "message_ok": _(
                    "We reviewed if the zip code of the receiver company is the same than this company"
                ),
            ...
            13: {
                ...
                "message": _(
                    "It looks like the CFDI attached is not registered in the SAT system or is cancelled."
                ),
    ...
    def action_open_archived_expenses(self):
        related = self.search([("l10n_mx_edi_expense_id", "=", self.id), ("active", "in", (False, True))])
        return {
            "name": _("Archived Expenses"),
            "view_mode": "tree,form",
            "res_model": "hr.expense",
            "view_id": False,
            "type": "ir.actions.act_window",
    ...
    def l10n_mx_edi_create_expense_invoice(self):
        ...
        for exp in expenses - to_omit:
            ...
            )  # noqa pylint: disable=context-overridden
    ...
    def l10n_mx_edi_get_invoice_data_get_lines(self, inv):
        ...
        for line in inv.get("lines", []):
            ...
            if uom_id and uom_id.category_id != prod.browse(product).uom_id:
                product = False
    ...
    def l10n_mx_edi_get_invoice_data(self, inv):
        ...
        usage = dict(invoice._fields["l10n_mx_edi_usage"].selection)
        return {
            "partner_id": self.partner_id.id,
            "ref": f'{inv["serie"]}{inv["number"]}',
            "invoice_payment_term_id": acc_pay_term.id,
            "l10n_mx_edi_payment_method_id": payment_method.id,
            "l10n_mx_edi_usage": inv["use_cfdi"] if inv["use_cfdi"] in usage else "",
    ...
    def dumps():
        ...
        import json  # pylint: disable=redefined-outer-name,reimported,import-outside-toplevel
    ...
    def l10n_mx_edi_action_ack(self):
        for record in self.filtered(lambda e: e.state == "downloaded"):
            ...
            functional_details = json.loads(record.l10n_mx_edi_functional_details or "{}")
            ...
            if fail:
                record.message_post(
                    ...
                    % (fail.get("title", ""), fail.get("message", ""))
                )
                values.update(
                    {
                        "active": False,
                        "l10n_mx_edi_expense_id": record._get_duplicated_expenses().id,
                    }
                )

            # If the CFDI is a payment complement, archive the expense
            analysis = json.loads(record.l10n_mx_edi_analysis or "{}")
            invoices = analysis.get("invoices")
            if invoices and invoices[0].get("document_type") == "P":
                record.message_post(body=_("This expense was archived because its CFDI is a payment complement"))

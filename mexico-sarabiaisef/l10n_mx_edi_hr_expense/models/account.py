...
class AccountJournal(models.Model):
    ...
    l10n_mx_edi_employee_ids = fields.Many2many(
        "hr.employee",
        "employee_journal_petty_cash",
        "journal_id",
        "employee_id",
        help="Employees that could to use this journal.",
    )
    is_petty_cash = fields.Boolean(help="Defines if this journal is to be treated as a Petty Cash")
    petty_cash_reserve = fields.Float(
        help="Amount to be allocated as permanent reserve that is to be fulfilled whenever possible"
    )
    ...
    def parse_string_to_float(self, string):
        return float(
            parse_decimal(
                string.replace((self.currency_id or self.company_id.currency_id).symbol, ""),
                locale=(self.env.context.get("lang") or self.env.user.company_id.partner_id.lang or "en_US"),
            )
        )
    ...
    def get_journal_dashboard_datas(self):
        ...
        res["petty_cash_reserve"] = formatLang(
            self.env, currency.round(self.petty_cash_reserve) + 0.0, currency_obj=currency
        )
        account_balance = self.parse_string_to_float(res["account_balance"])
        to_replenish = currency.round(self.petty_cash_reserve - account_balance) + 0.0
        res["amount_to_replenish_no_currency"] = to_replenish
        res["amount_to_replenish"] = formatLang(self.env, to_replenish, currency_obj=currency)
        return res
    ...
    def _prepare_liquidity_account_vals(self, company, code, vals):
        ...
        account set via config parameter to be used as template, assign the
        ...
        template_account_id = (
            self.env["ir.config_parameter"].sudo().get_param("l10n_mx_edi_hr_expense.template_account_employee")
        )
        ...
        res["code"] = account_model._search_new_account_code(company, digits, prefix)
        return res

...
class L10nMxEdiReclassifyJournalEntries(models.TransientModel):
    ...
    account_id = fields.Many2one("account.account", help="An expense account is expected")
    product_id = fields.Many2one(
        "product.product", domain=[("can_be_expensed", "=", True)], help="Product to assign in the expenses related"
    )
    ...
    def default_get(self, fields_list):
        ...
        if not active_ids or active_model != "account.move":
            return res
        if self.env["account.move"].browse(active_ids).mapped("move_type")[0] not in ["in_invoice", "in_refund"]:
            raise UserError(_("You can only reclassify journal entries for vendor bills"))
        return res
    ...
    def reclassify_journal_entries(self):
        ...
        if model == "hr.expense.sheet":
            records = self.env[model].browse(records).mapped("expense_line_ids").ids
        if model in ["hr.expense", "hr.expense.sheet"]:
            for exp in self.env["hr.expense"].browse(records):
                state = exp.state
                exp.state = "draft"
                exp.product_id = self.product_id if self.product_id else exp.product_id
                exp.account_id = self.account_id if self.account_id else exp.account_id
                ...
                line.account_id = self.account_id if self.account_id else line.account_id
            ...
            for inv in self.env["account.move"].browse(records):
                ...
                for line in inv.invoice_line_ids:
                    line.account_id = self.account_id if self.account_id else line.account_id
                    line.product_id = self.product_id if self.product_id else line.product_id
                if not inv.move_id:
                    continue
                ...
                lines = inv.move_id.line_ids.filtered(
                    lambda l: l.account_id.user_type_id.type != "payable" and l.account_id not in tax_accounts
                )
                for line in lines:
                    line.account_id = self.account_id if self.account_id else line.account_id
                    line.date = self.date if self.date else line.date

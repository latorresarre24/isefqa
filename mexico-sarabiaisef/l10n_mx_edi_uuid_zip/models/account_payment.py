    ...
    def _l10n_mx_edi_uuid_zip_post(self):
        ...
        for payment in self.filtered(lambda r: r.company_id.country_id == self.env.ref('base.mx')):
            ...
            action = invoice_zip.with_context(
                active_ids=payment.reconciled_invoice_ids.ids, active_model='account.move').run()
            if not action:
                continue
            data = action['data']
            ...
            if attach:
                ...
                payment.message_post(body="%s %s" % (_('Invoices CFDI XMLs'), _('updated')), subtype_xmlid=msg_subt)
                continue
            ...
            payment.message_post(body=_('Invoices CFDI XMLs'), attachment_ids=attach.ids, subtype_xmlid=msg_subt)

...
class PointOfSale(TransactionCase):
    ...
    def setUp(self):
        super().setUp()
        self.uid = self.env.ref('base.user_demo')
        ...
        self.company = self.env.ref('l10n_mx.demo_company_mx')
        # Partner with RFC
        self.partner1 = self.env.ref('base.res_partner_12')
        self.partner1.commercial_partner_id.vat = 'VCO120608KR7'
        account_receivable = self.partner1.property_account_receivable_id.sudo()
        account_receivable = account_receivable.search([
            ('code', '=', account_receivable.code), ('company_id', '=', self.company.id)])
        self.partner1.property_account_receivable_id = account_receivable
        # Partner without RFC
        ...
        self.partner2.property_account_receivable_id = account_receivable
        self.partner2.commercial_partner_id.property_account_receivable_id = account_receivable
        ...
        self.tax_16 = self.env['account.tax'].sudo().create({
            'name': 'tax_16',
            'amount_type': 'percent',
            'amount': 16,
            'type_tax_use': 'sale',
        ...
        self.tax_10_negative = self.tax_16.create({
    ...
    def base_test(self, pac_name):
        ...
        self.company.sudo().l10n_mx_edi_pac = pac_name or 'finkok'
        ...
        self.session = self.create_session()
        self.create_order()
        self.company.sudo().l10n_mx_edi_pac = False
    ...
    def test_005_no_consider_refund_without_cfdi_origin(self):
        ...
        vat = company.vat
        company.sudo().vat = False
        self.session.action_pos_session_closing_control()
        self.assertEqual(self.session.l10n_mx_edi_pac_status, "retry", self.session.message_ids.mapped('body'))
        company.sudo().vat = vat
        session2 = self.create_session()
        refund = order.refund()
        self.assertTrue(refund.get('res_id'), 'The refund was not created')
        refund = self.pos_order_obj.browse(refund.get('res_id'))
    ...
    def test_007_l10n_mx_edi_pos(self):
        ...
        self.company.sudo().l10n_mx_edi_pac_username = 'luis_t@vauxoo.com'
        self.company.sudo().l10n_mx_edi_pac_password = 'VAU.2021.SW'
        self.base_test('sw')
    ...
    def test_both_taxes0(self):
        ...
        tax0 = self.env['account.tax'].sudo().create({
            'name': 'IVA 0% PoS',
            'description': 'IVA(0%)',
            'amount': 0,
            'amount_type': 'percent',
        ...
        ieps0 = self.env['account.tax'].sudo().create({
            'name': 'IEPS 0% PoS',
            'description': 'IEPS(0%)',
            'amount': 0,
            'amount_type': 'percent',
    ...
    def test_l10n_mx_edi_pos_33(self):
        """ Make sure all POS Flow is possible with the main pac: finkok"""
        self.env["ir.config_parameter"].sudo().create({
            "key": "l10n_mx_edi_pos_version",
            "value": "4.0"
        })
        self.base_test('finkok')

    def create_session(self):
        return self.env['pos.session'].with_env(
            self.env(user=self.uid)).create({
                'user_id': self.uid,
                'config_id': self.env.ref('l10n_mx_edi_pos.pos_config_main_mx').id,
            })
